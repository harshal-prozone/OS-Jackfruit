...
if [[ -r /etc/os-release ]]; then
    ...
    case "${ID:-}" in
        ...
        debian)
            case "${VERSION_ID:-}" in
                11|12) ok "Debian ${VERSION_ID} (bullseye/bookworm) detected — treating as compatible." ;;
                *) warn "Unrecognised Debian version: ${VERSION_ID:-unknown}. Proceeding anyway." ;;
            esac
            ;;
        *)
            fail "Unsupported distro: ${ID:-unknown}. Use Ubuntu 22.04/24.04 or Debian 11/12."
            ;;
    esac
else
